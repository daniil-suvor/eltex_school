...
int change_bytes(int num, int n, int m); // swaps the n-th and m-th byte

int main() {
    int num = 0xABCDEFBB;
    print_bytes(num);
    printf("\n");
    num = change_bytes(num, 3, 1);
    print_bytes(num);
    return 0;
}
...
int insert_byte(int num, int n, int ins_byte) {
    ins_byte = ins_byte << n*LEN_BYT;  // shift one byte to the left
...
int change_bytes(int num, int n, int m) {
